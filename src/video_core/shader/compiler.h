...
private:
    ...
    void Compile_SanitizedMul(
        Xbyak::Xmm src1, Xbyak::Xmm src2,
        Xbyak::Xmm
            scratch); ///< Compiles a `MUL src1, src2` operation, properly handling the PICA
                      /// semantics when multiplying zero by inf. Clobbers `src2` and `scratch`.
    void Compile_EvaluateCondition(Instruction instr);
    void Compile_UniformCondition(Instruction instr);
    void Compile_Return(); ///< Emits the code to conditionally return from a subroutine envoked by
    ...
    void FindReturnOffsets(); ///< Analyzes the entire shader program for `CALL` instructions before
                              /// emitting any code, identifying the locations where a return needs
                              /// to be inserted.
    ...
    const std::array<u32, MAX_SWIZZLE_DATA_LENGTH>* swizzle_data;
    std::array<Xbyak::Label, MAX_PROGRAM_CODE_LENGTH>
        instruction_labels; ///< Mapping of Pica VS instructions to pointers in the emitted code
    std::optional<Xbyak::Label>
        loop_break_label; ///< Label pointing to the end of the current LOOP block. Used by the
                          /// BREAKC instruction to break out of the loop.
    std::vector<unsigned> return_offsets; ///< Offsets in code where a return needs to be inserted
    unsigned program_counter{};           ///< Offset of the next instruction to decode
    bool looping{}; ///< True if compiling a loop, used to check for nested loops
