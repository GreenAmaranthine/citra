...
struct State {
    ...
    struct Lighting {
        union LutEntry {
            u32 raw;                    ///< Used for raw access
            BitField<0, 12, u32> value; ///< LUT value, encoded as 12-bit fixed point, with 12
                                        ///< fraction bits. 0.0.12 fixed point
    ...
    struct ImmediateModeState {
        Shader::AttributeBuffer
            input_vertex;        ///< Used to buffer partial vertices for immediate-mode rendering.
        u32 current_attribute{}; ///< Index of the next attribute to be loaded into `input_vertex`.
        bool reset_geometry_pipeline{}; ///< Indicates the immediate mode just started and the
                                        /// geometry pipeline needs to reconfigure
    } immediate;
