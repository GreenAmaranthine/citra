...
void to_json(nlohmann::json& json, const Room::Member& member) {
    ...
    json["gameName"] = member.program_name;
    json["gameId"] = member.program_id;
}

void from_json(const nlohmann::json& json, Room::Member& member) {
    member.name = json.at("name").get<std::string>();
    member.program_name = json.at("gameName").get<std::string>();
    member.program_id = json.at("gameId").get<u64>();
}
...
void to_json(nlohmann::json& json, const Room& room) {
    ...
    json["port"] = room.port;
    json["name"] = room.name;
    json["preferredGameName"] = room.preferred_program;
    json["preferredGameId"] = room.preferred_program_id;
...
void from_json(const nlohmann::json& json, Room& room) {
    ...
    room.preferred_program = json.at("preferredGameName").get<std::string>();
    room.preferred_program_id = json.at("preferredGameId").get<u64>();
...
void RoomJson::SetRoomInformation(const std::string& uid, const std::string& name, const u16 port,
                                  ...
                                  const u64 preferred_program_id) {
    room.name = name;
    room.UID = uid;
    room.port = port;
    ...
    room.preferred_program = preferred_program;
    room.preferred_program_id = preferred_program_id;
}
void RoomJson::AddMember(const std::string& nickname, const MacAddress& mac_address,
                         const u64 program_id, const std::string& program_name) {
