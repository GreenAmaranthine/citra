...
{
    return headers.headers.find(key) != headers.headers.end();
}

inline const char *get_header_value(const Headers &headers, const char *key,
                                    const char *def = nullptr)
{
    auto it = headers.headers.find(key);
...
inline bool read_headers(Stream &strm, Headers &headers)
{
    ...
    for (;;)
    {
        ...
        if (std::regex_match(reader.ptr(), m, re))
        {
            ...
            headers.headers.emplace(key, val);
        }
    }
...
{
    for (const auto &x : info.headers.headers)
    {
                                              ...
                                              const char *content_type)
{
    ...
    req.headers.headers.emplace("Content-Type", content_type);
    req.body = body;
