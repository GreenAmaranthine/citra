...
dylibbundler -b -x "${REV_NAME}/citra-qt.app/Contents/MacOS/citra" -cd -d "${REV_NAME}/libs" -p "@executable_path/libs/"
...
$BREW_PATH/opt/qt5/bin/macdeployqt "${REV_NAME_ALT}citra.app" \
    -executable="${REV_NAME_ALT}citra.app/Contents/MacOS/citra"
...
do
    ...
    cp "$BREW_PATH/opt/qt5/lib/$SC_FRAMEWORK_PART" "${REV_NAME_ALT}citra.app/Contents/Frameworks/$SC_FRAMEWORK_PART"
    ...
    do
        ...
        install_name_tool -change \
            ...
            "${REV_NAME_ALT}citra.app/Contents/Frameworks/$SC_FRAMEWORK_PART"
        install_name_tool -change \
...
do
    install_name_tool -id @executable_path/../$macos_lib "${REV_NAME_ALT}citra.app/Contents/$macos_lib"
    for macos_lib2 in "${macos_libs[@]}"
    do
        RM_FRAMEWORK_PART=$macos_lib2.framework/Versions/$QT_VERSION_NUM/$macos_lib2
        install_name_tool -change \
            $QT_BREWS_PATH/lib/$RM_FRAMEWORK_PART \
            @executable_path/../Frameworks/$RM_FRAMEWORK_PART \
            "${REV_NAME_ALT}citra.app/Contents/$macos_lib"
        install_name_tool -change \
            "$BREW_PATH/opt/qt5/lib/$RM_FRAMEWORK_PART" \
            @executable_path/../Frameworks/$RM_FRAMEWORK_PART \
            "${REV_NAME_ALT}citra.app/Contents/$macos_lib"
...
do
    ...
    otool -L "${REV_NAME_ALT}citra.app/Contents/Frameworks/$macos_lib.framework/Versions/$QT_VERSION_NUM/$macos_lib"
done
...
chmod +x ${REV_NAME_ALT}citra.app/Contents/MacOS/citra
